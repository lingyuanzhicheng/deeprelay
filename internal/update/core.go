...
func getDownloadFilename() (string, error) {
	...
	case "windows":
		switch arch {
		case "386":
			return "deeprelay-windows-x86.zip", nil
		case "amd64":
			return "deeprelay-windows-x86_64.zip", nil
		}
	case "darwin":
		switch arch {
		case "amd64":
			return "deeprelay-darwin-x86_64.zip", nil
		case "arm64":
			return "deeprelay-darwin-arm64.zip", nil
		}
	case "linux":
		switch arch {
		case "386":
			return "deeprelay-linux-x86.zip", nil
		case "amd64":
			return "deeprelay-linux-x86_64.zip", nil
		case "arm":
			return "deeprelay-linux-armv7.zip", nil
		case "arm64":
			return "deeprelay-linux-arm64.zip", nil
		}
	}
